...
public class Exercise1OLD extends BaseTest {


    //1. Open test site by URL - in parent class

    @Test(priority = 0, description = "1. Open test site by URL - in parent class")
    @Description(value = "какой-то дескрипшн от аллюр")
    @Step
    public void assertTitle() {
        ...
        // аттачи
        final String inputParams = "{a:'a'}";
        Allure.addAttachment("browser title", browserTitle);
        Allure.step("jjjjjjj step");


        softly.assertThat(browserTitle).as("Incorrect browser title").isEqualTo("HomeFAIL Page");
        softly.assertAll();
    }
